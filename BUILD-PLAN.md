...
## Phase I · The voice memo

**Added 2026-09-22, and the only phase here that un-decides something rather
than building on it.** Ben asked for a switch on the reflect step — off by
default — that also keeps the recording of a spoken answer. The design is
...
an answer on its own. That is the part of D1 worth keeping: **text is the
record**, and audio is a short-lived extra the diary can lose without losing
the entry.

- [ ] **I.1 The promise, before the code that breaks it.** D16 into
...
- [ ] **I.2 The column, the bucket, and four policies.** One new migration;
  ...
  *Done when:* `pnpm test:db` proves a second anonymous user cannot read,
  ...
  fact I.3 is built on, and a test written expecting a cascade would pass for
  the wrong reason.

- [ ] **I.3 Deletion and the sweep — BEFORE anything can be captured.** The
  order looks inverted and is not: the moment I.4 ships, files accumulate that
...
- [ ] **I.4 Capture: the switch, the WAV, the upload.** `Switch` exists and is
  ...
  **The switch is read before the microphone opens, and locked while it runs.**
  ...
  iOS Safari surface Phase F’s own checkpoint already says to budget for, and this
...
- [ ] **I.5 Playback, which is mostly already built.** `VoiceNote` (§7.19) is
...
thing that sends I.4 back for `MediaRecorder`.
...
| **Thirty days or ninety** — how long a voice memo is kept. Thirty is recommended, with the argument in [docs/VOICE-MEMO.md](docs/VOICE-MEMO.md) §2 | **I.2, and everything after it.** It has a clock on it: a retention window changed later changes rows that were written under the old one, so it is settled before the migration or not cheaply at all |
