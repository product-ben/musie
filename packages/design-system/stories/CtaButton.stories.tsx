    ...
    docs: {
      description: {
        component: [
          ...
          '',
          '**One primary per unit, and it is the way on.** Ben, 2026-09-24, as a rule',
          'for the system. Within one semantic unit of the interface — a screen, a',
          'wizard step, a panel, a frame — and within one STATE of that unit, the action',
          'that carries the person onward takes `primary`. At most one does; a state with',
          'no way onward has no primary at all.',
          '',
          'Everything else in the unit is `secondary` or `ghost`, however important it',
          'is: going back, changing your mind, opening the other way in, retrying',
          'something that failed. Importance is not the test — direction is. *Back* is',
          'never primary even when it is the likeliest press.',
          '',
          'It is a rule about the unit **and the state**, which is what makes it usable',
          'on a component that changes what it shows: when a frame swaps a viewfinder for',
          'a form, the way on swaps with it, and the primary moves. Two primaries visible',
          'at once means the unit has been drawn wrongly, not that the rule bends.',
          '',
          'It does NOT settle conflict B12 / open question 7, which asks which ACCENT a',
          'screen should reach for. This one says when a filled treatment is owed at all.',
          '',
