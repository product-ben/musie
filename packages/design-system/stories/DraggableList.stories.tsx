...
const meta = {
    ...
    docs: {
      description: {
        component: [
          ...
          '',
          '**DraggableList — `DropHints` is not exported from `src/index.ts`.** The',
          'type of the public `dropHints` prop cannot be named by a consumer through',
          'the package entry point; it is exported from the module but missing from',
          'the barrel. Already recorded in reference/INVENTORY.md §3.',
          '',
          '**DraggableList — every default string is English while the earlier**',
          '**components default to German.** `emptyHeadline`, `emptyText`,',
          '`listeningLabel`, `hearingLabel`, `itemNoun`, `label` and all four',
          '`dropHints` are English, and the header says so deliberately. IconButton',
          'and CtaButton default to `\'Wird geladen\'`. See the session-level language',
          'entry in OPEN-QUESTIONS.md.',
          '',
          '**DraggableList — the row action copy has no prop at all.** "Discard",',
          '"Save", "Delete" and "Edit", the handle and chevron `aria-label`s, and',
          'every keyboard live-region announcement are hardcoded English in the JSX.',
          'A consumer who localises everything the props expose still ships four',
          'English buttons.',
          '',
  ...
  argTypes: {
    ...
    itemNoun: { control: 'text', description: 'What one item is called, for every control’s accessible name.' },
    label: { control: 'text', description: 'Accessible name for the list itself.' },
    dropHints: { control: false, description: 'The drag hint’s wording. Defaults are English; the consumer localises.' },
