    ...
    docs: {
      description: {
        component: [
          ...
          '',
          '**DraggableList — a11y, still not verified with a screen reader.** The',
          'keyboard equivalents (Space or Enter lifts, arrows move, M merges,',
          'Escape cancels) are the only way to reach the drag states without a',
          'pointer, and no session so far has exercised them with assistive tech.',
          '',
          '**PARTLY ANSWERED in F.5 — focus survives a merge and a delete now.**',
          'Both end the row the operated control sits in, so the document used to',
          'drop focus to `<body>`: the second merge started from the top of the',
